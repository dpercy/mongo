...
assert.eq(3, masterDB[collection].getIndexes().length);

// Wait for the secondary to get the index entry
assert.soon( 
    function() { return 3 == masterDB[collection].getIndexes().length; }, 
    "index not created on secondary (prior to drop)", 240000 );

jsTest.log("Index created and getIndexes() entry exists on secondary");
...
printjson( masterDB[collection].getIndexes() );
...
assert.soon( function() {
    ...
    printjson( secondDB[collection].getIndexes() );
    
    i++;
    return 1 === secondDB[collection].getIndexes().length;
